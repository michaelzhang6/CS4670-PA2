...
def compare_cv2_points(pnt1, pnt2):
    if not np.isclose(pnt1.pt[0], pnt2.pt[0], rtol=1e-3, atol=1e-5):
        return False
    if not np.isclose(pnt1.pt[1], pnt2.pt[1], rtol=1e-3, atol=1e-5):
        return False
...
try_this(1, HKD.computeHarrisValues, [
         loaded['a'], loaded['b']], compare_array, grayImage)
...
try_this('5 and/or 6', MFD.describeFeatures,
